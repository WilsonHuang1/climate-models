...
class TemperatureElevation(Plot):
    ...
    def make_fig(self):
        """Create a temperature plot at specific elevation using cartopy and matplotlib."""
        ...
        try:
            ...
            # Add title with elevation level information
            period_years = f"{4000 + int(self.time_periods[0]) * 10}-{4000 + (int(self.time_periods[0]) + 1) * 10}"
            elevation_level = 30 - self.elevation  # Convert from 0-based index to 1-based level
            plt.title(f"{self.title} (Level {elevation_level})\n{period_years}")
            
            # Add statistics text box with elevation information
            stats_text = (f"Elevation Level: {elevation_level}\n"
                        f"Mean: {mean:.2f} K\n"
                        f"Std Dev: {std:.2f} K\n"
